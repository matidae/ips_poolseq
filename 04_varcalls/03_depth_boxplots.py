...
def make_boxplots(stats_file):
    ...
    group_prefixes = set([i[:6] for i in samples])
    ...
    for prefix, samples_group in zip(group_prefixes, grouped_samples):         
        ...
        for box in bp['boxes']:
            box.set(facecolor="lightseagreen", alpha=0.7)
            box.set(edgecolor="darkslategrey", linewidth=1.5) 
        for median in bp['medians']:
            median.set(color="darkslategrey", linewidth=2) 
        for whisker in bp['whiskers']:
            whisker.set(color="darkslategrey", linewidth=1.5)
        for cap in bp['caps']:
            cap.set(color="darkslategrey", linewidth=1.5)
...
def make_total_boxplots(genic_stats, intergenic_stats):
    ...
    for idx, row in combined.iterrows():
        box_data.append({
            ...
            'whislo': row['whislo'],
            'whishi': row['whishi'],
            'fliers': []  # No fliers since we only have summary stats
        })
