...
bcftools view "$work_dir/ips_merged.2.vcf.gz" -m2 -M2 -v snps -e 'INFO/MQ<20 || QUAL<20' \
  -Oz -o "$work_dir/ips.biallelic_q20_m20.vcf.gz" 

# Exclude all SNPs where min(p) <= (1 - min_MAF) and max(p) >= min_MAF
python3 01aux_filter_MAF.py
...
bcftools view -h "$work_dir/ips.biallelic_q20_m20.maf05.gt.vcf.gz" | tee "$work_dir/genic_variants.vcf" "$work_dir/intergenic_variants.vcf" > /dev/null
