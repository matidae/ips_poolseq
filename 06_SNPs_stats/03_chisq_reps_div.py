...
def aggregate_SNPs_by_coverage_bin(stats, dcat, p_value):
...
def main():
    ...
    with open(m_and_z_in, "r") as m_and_z_fh, open(snpdev_out, "w") as snpdev_fh:
        ...
        for line in m_and_z_fh:
            ...
            if snpstats[0]>0:  # If there were valid comparisons
                ...
                aggregate_SNPs_by_coverage_bin(stats, dcat, p_value)             
    ...
    with open(cov_bin_out, "w") as cov_bin_fh:
        cov_bin_fh.write("depth_start\tSNP_count\tpval_lt_0.01\n")
        for dcat in sorted(stats):
            coverage_start = dcat * 500
            num_snps = stats[dcat][0]
            prop = stats[dcat][1] / num_snps
