...
# "sync_score" per SNP measuring if it follows the population-avg fluctuation.
#
# Inputs:
...
#   - _sync_scores.tsv         : sync_score and rank per SNP
#   - _interval_types.tsv      : interval types (up/down) inferred from mean dz
#----------------------------------------------------------------------
...
prefixes = [os.path.basename(f).split(".")[1] for f in thinned_snps_in]
...
#Extracts year from timepoint label: 'WFIN_E_2022' -> '2022'
def simplify_interval(y1, y2):   
    year1 = str(y1).split("_")[-1]
    year2 = str(y2).split("_")[-1]
    return f"{year1}-{year2}"

# Computes mean dz, t-tests, correlations, and saves output files.
def analyze_and_save(dznext, dz_per_snp, years, prefix, snp_zbars, sync_scores, interval_types):
    # - Save dz per SNP per interval
    dz_file = prefix + "_deltaZ_per_snp.tsv"
    with open(dz_file, "w", newline="") as f:
        writer = csv.writer(f, delimiter="\t")
        writer.writerow(["SNP"] + [simplify_interval(years[i], years[i+1]) for i in range(len(years)-1)])
    ...
    with open(mean_file, "w", newline="") as f:
        ...
        for i, year in enumerate(years[:-1]):
            ...
            label = simplify_interval(year, years[i+1])
    ...
    with open(corr_file, "w", newline="") as f:
        writer = csv.writer(f, delimiter="\t")
        intervals = [simplify_interval(years[i], years[i+1]) for i in range(len(years)-1)]
    ...
    # - Sync score per SNP
    sync_score_file = prefix + "_sync_scores.tsv"
    with open(sync_score_file, "w", newline="") as f:
        writer = csv.writer(f, delimiter="\t")
        writer.writerow(["SNP", "sync_score", "mean_z", "rank"])
        sorted_snps = sorted(sync_scores.items(), key=lambda x: x[1], reverse=True)
...
def compute_sync_scores(dznext, dz_per_snp, years):
    # Infer interval types
    interval_types = []
    for year in years[:-1]:
        mean_dz = sum(dznext[year]) / len(dznext[year])
        interval_types.append("up" if mean_dz > 0 else "down")
    # Score each SNP
    sync_scores = {}
    for snp, dz_list in dz_per_snp.items():
        sync_score = 0.0
        for j, dz in enumerate(dz_list):
            sync_score += dz if interval_types[j] == "up" else -dz
        sync_scores[snp] = sync_score
    return sync_scores, interval_types
...
def main():
    ...
    for pre in prefixes:
        ...
        output_prefix = f"{work_dir}/{pre}.fluctuating"
        z_file = f"{af_dir}/z_year.{pre}.tsv"
        rsnps = read_thinned_file(thinned_file)
        dznext, dz_per_snp, years, snp_zbars = read_z_file(z_file, rsnps)
        sync_scores, interval_types = compute_sync_scores(dznext, dz_per_snp, years)
        analyze_and_save(dznext, dz_per_snp, years, output_prefix, snp_zbars, sync_scores, interval_types)
